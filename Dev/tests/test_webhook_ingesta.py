...
@pytest.mark.parametrize("alert_type,expected", [
    ...
    ("api_latency_high", "P1"),
    ("response_timeout", "P1"),
    ("degraded_throughput", "P1"),
    ("DISK_FULL", "P2"),
    ("unknown_signal", "P2"),
    ("", "P2"),
])
def test_classify_severity_rules(alert_type, expected):
    """classify_severity infers P0/P1 from keywords, P2 otherwise."""
    assert models.classify_severity(alert_type) == expected


def test_ingest_auto_classifies_p0_when_severity_omitted(aws_services):
    """A 503 alert with no explicit severity is auto-classified P0 (15-min SLA)."""
    table, _ = aws_services
    handler = get_handler()
    ...
    meta = table.get_item(
        Key={"PK": keys.ticket_pk(body["ticket_id"]), "SK": keys.meta_sk()}
    )["Item"]
    assert meta["severity"] == "P0"


def test_ingest_auto_classifies_p1(aws_services):
    """A latency alert with no explicit severity is auto-classified P1."""
    table, _ = aws_services
    handler = get_handler()

    resp, body = _ingest(handler, service="checkout", alert_type="api_latency_high")
    assert resp["statusCode"] == 201
...
def test_ingest_explicit_severity_overrides_classification(aws_services):
    """An explicit severity always wins over the keyword classifier."""
    table, _ = aws_services
    handler = get_handler()

    # HTTP_503 would classify P0, but the caller forces P2.
    resp, body = _ingest(handler, service="payments", alert_type="HTTP_503", severity="P2")
    assert resp["statusCode"] == 201
