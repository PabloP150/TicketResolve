...
# ---------------------------------------------------------------------------
# Auto-severity classification (US-01 / US-03)
#
# When a monitoring webhook does not declare an explicit severity, the system
# infers it from the alert_type using keyword rules evaluated most-severe-first
# (P0 → P1 → P2 fallback). Rules are data-driven (a dict of keyword tuples) so
# they can be audited, tuned, or externalised to configuration later without
# touching the matching logic.
# ---------------------------------------------------------------------------

SEVERITY_KEYWORDS: Final[dict[str, tuple[str, ...]]] = {
    "P0": (
        "down", "outage", "unavailable", "offline", "crash", "fatal",
        "critical", "503", "500", "5xx", "data_loss", "breach",
    ),
    "P1": (
        "latency", "slow", "degraded", "timeout", "error", "high_cpu",
        "high_memory", "saturation", "throttle", "elevated",
    ),
}


def classify_severity(alert_type: str) -> str:
    """Infer a severity from an alert_type string using keyword rules.

    Matching is case-insensitive and substring-based, evaluated most-severe
    first (P0 before P1). Returns DEFAULT_SEVERITY (P2) when nothing matches,
    so an unknown alert is never silently treated as critical.
    """
    needle = (alert_type or "").strip().lower()
    for severity in ("P0", "P1"):
        if any(keyword in needle for keyword in SEVERITY_KEYWORDS[severity]):
            return severity
...
ESCALATED_STATUS: Final[str] = "ESCALATED"

RESOLVABLE_STATUSES: Final[tuple[str, ...]] = ("OPEN", "ACK", "ESCALATED")

# Actor recorded on events produced by the system itself (e.g. the automatic
# SLA-breach escalation worker) rather than by a human API caller.
