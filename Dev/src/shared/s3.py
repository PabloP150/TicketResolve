...
def generate_presigned_get(bucket: str, s3_key: str, expires: int = 300) -> str:
    """
    Generate a presigned GET URL for an explicit bucket/key pair.

    Unlike generate_presigned_get_url (which targets the attachments bucket via
    ATTACHMENTS_BUCKET), this lets callers download from any bucket they own —
    used by the reporte_pdf worker to hand back a temporary link to a monthly
    report stored in the reports bucket.
    """
    client = get_s3_client()
    url = client.generate_presigned_url(
        "get_object",
