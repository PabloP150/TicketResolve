...
Routes handled:
  ...
  PATCH  /api/v1/incidents/{id}/assignee     → reassign_ticket
  POST   /api/v1/reports                     → request_report (US-06, async PDF)
"""
import logging
import os
import re
...
def lambda_handler(event: dict, context) -> dict:
    ...
    try:
        ...
        # POST /api/v1/reports — trigger async monthly report (US-06)
        if method == "POST" and raw_path == _REPORTS:
            body = httputil.parse_body(event)
            result, status = service.request_report(body)
            return httputil.accepted(result)
