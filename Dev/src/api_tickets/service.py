...
def update_status(ticket_id: str, body: dict) -> tuple[dict, int]:
    ...
    # Emit a domain event for async notification fan-out (US-05): a resolution
    # confirmation to the reporter, or an alert to the next tier on a manual
    # escalation. Best-effort and AFTER the commit — a queue failure must never
    # roll back an already-persisted transition.
    if target_status in (models.RESOLVED_STATUS, models.ESCALATED_STATUS):
        try:
            events.enqueue(
                target_status,
                ticket_id,
            ...
            logger.exception("Failed to enqueue %s event for %s", target_status, ticket_id)
...
# ---------------------------------------------------------------------------
# Request monthly report  (US-06 — admin trigger)
# ---------------------------------------------------------------------------
...
def _get_lambda_client() -> Any:
    """Return (lazily initialised) boto3 Lambda client (for async invoke)."""
...
def request_report(body: dict) -> tuple[dict, int]:
    """Trigger asynchronous generation of a monthly PDF report (US-06).

    The heavy scan + render runs in the reporte_pdf worker, invoked
    asynchronously (InvocationType='Event') so this admin call returns
    immediately with 202 and never degrades the transactional path.

    Body (optional): {"month": "YYYY-MM"}. Defaults to the current month.

    The target function name is read from REPORT_FUNCTION_NAME. When it is
    unset (local dev without the worker wired) the request is still accepted
    (202) and simply records that no async dispatch occurred — callers can
    invoke the worker directly in that environment.
    """
    body = body or {}
    month = body.get("month")
    if month is not None and not _MONTH_RE.match(str(month)):
        raise models.ValidationError("'month' must be in 'YYYY-MM' format.")

    function_name = os.environ.get("REPORT_FUNCTION_NAME")
    dispatched = False
    if function_name:
        ...
        dispatched = True
        logger.info("Dispatched report generation (month=%s) to %s", month, function_name)
    else:
        logger.warning("REPORT_FUNCTION_NAME unset; report request accepted but not dispatched.")
...
def ingest_alert(body: dict) -> tuple[dict, int]:
    ...
    # Auto-severity (US-01/US-03): an explicit severity in the payload always
    # wins; otherwise infer it from the alert_type keyword rules so monitoring
    # alerts are prioritised automatically instead of defaulting to P2.
    explicit_severity = body.get("severity")
    if explicit_severity:
        severity = models.validate_severity(explicit_severity)
    else:
        severity = models.classify_severity(alert_type)
