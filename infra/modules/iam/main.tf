...
# ===========================================================================
# GitHub Actions OIDC provider + CI runner role (Deliverable C).
# The trust policy is scoped to this repository's exact subject claims
# (main branch ref + dev/staging environments) — no wildcard subject, so a
# fork's PR can never assume this role.
# ===========================================================================
...
# Deploy permissions. A CI/CD role that runs `terraform apply` must be able to
# create resources that do not exist yet, so it cannot reference their ARNs —
# Resource is "*" by necessity, but Actions are enumerated per service (no
# "*" action) and the role is assumable ONLY from this repo via OIDC.
